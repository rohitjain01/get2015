...
/**
 * This class contains functionality of rooms i.e allocation of room , search of
 * guest name and many more
 * 
 * @author Akshat
 *
 */
public class RoomsAllotment {
	 ...
	 *            of customer
	 * @param i
	 *            ittration variable in key function
	 * @return
	 */
	...
	public void allocateRoom() {
		...
		int i = 0;
		while (i < noOfRooms) {
			int key = generateKey(customer.getAge(), i);
			if (roomList.get(key) == null) {
				roomList.put(key, customer.getName());
				System.out.println("room " + (key + 1) + " is allocated to "
						+ roomList.get(key));
				countOfGuest++;
				break;
			}
	...
	public void searchByName(String nameOfGuest) {
		...
		for (Entry<Integer, String> records : roomList.entrySet()) {
			if (records.getValue().equalsIgnoreCase(nameOfGuest)) {
				System.out.println(records.getValue()
						+ " is currently living in room "
						+ (records.getKey() + 1));
				isNameFound = true;
			}
		}
	...
	public void showBookingDetail() {
		for (Entry<Integer, String> records : roomList.entrySet()) {
			System.out.println("Room Number: " + (records.getKey() + 1)
					+ " ---> " + records.getValue());
		}
	}
