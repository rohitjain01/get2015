			...
			do
			{
				...
				switch(choice)
				{
					case 1:
						...
						roomNumbers = rooms.keys();	// roomnumvers hold all the keys (room numbers) from hash table
					...
					case 2:
						...
						i = 0;
						while(flag) // if room is not find
						{
							for(int j = i; j <= (numberOfRooms - 1); j++)// than check rooms status from starting
							{
								if(rooms.get(j).equals("")) // if empty than aalocate
								{
									rooms.put(j, name);
